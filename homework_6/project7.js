...
function SimTimeStep(dt, positions, velocities, springs, stiffness, damping, particleMass, gravity, restitution) {
    ...
    for (let spring of springs) {
        ...
        let forceMag = stiffness * (dist - spring.rest);
    ...
    for (let i = 0; i < positions.length; i++) {
        ...
        forces[i] = forces[i].add(gravityForce).add(dampingForce);
    }

    // Integrate velocities and positions using explicit Euler
    for (let i = 0; i < positions.length; i++) {
    ...
    // Handle collisions with the bounding box [-1, 1]^3
    for (let i = 0; i < positions.length; i++) {
